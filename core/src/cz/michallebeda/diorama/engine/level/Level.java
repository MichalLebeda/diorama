...
import cz.michallebeda.diorama.engine.level.fx.Clouds;
import cz.michallebeda.diorama.engine.level.logic.Logic;
import cz.michallebeda.diorama.engine.level.logic.component.InitComponent;
...
import cz.michallebeda.diorama.engine.level.portal.Portals;
import cz.michallebeda.diorama.engine.physics.BoxFactory;
import cz.michallebeda.diorama.engine.physics.LevelContactListener;

public class Level {
    protected cz.michallebeda.diorama.engine.level.portal.Portals portals;
    protected MetaLevel metaLevel;
    protected cz.michallebeda.diorama.engine.level.Floor floor;
    protected cz.michallebeda.diorama.engine.level.object.GameObjects gameObjects;
    protected cz.michallebeda.diorama.engine.level.fx.Clouds clouds;
    protected World world;
    protected cz.michallebeda.diorama.engine.level.logic.Logic logic;
    protected BoxFactory boxFactory;
    protected PerspectiveCamera camera;
    protected cz.michallebeda.diorama.engine.level.logic.component.InitComponent initComponent;
    ...
    private Level(EngineGame game, MetaLevel metaLevel) {
        ...
        logic = new cz.michallebeda.diorama.engine.level.logic.Logic(metaLevel.getParentProject().getIdGenerator());

        initComponent = new InitComponent();
        logic.register(initComponent);

        gameObjects = new cz.michallebeda.diorama.engine.level.object.GameObjects(logic);
        portals = new cz.michallebeda.diorama.engine.level.portal.Portals(metaLevel, logic,
                boxFactory,
                resources);
    }
    ...
    public Level(MetaLevel metaLevel, EngineGame game, int width, int height) {
        ...
        floor = new cz.michallebeda.diorama.engine.level.Floor(game.getResources().getObjectAtlas().findRegion("ground"), width, height);
        clouds = new cz.michallebeda.diorama.engine.level.fx.Clouds(floor, game.getResources().getObjectAtlas());
    ...
    public Level(MetaLevel metaLevel, EngineGame game) {
        ...
        floor = new cz.michallebeda.diorama.engine.level.Floor();
...
//
    ...
    public void updateBasedOnPlayer(float delta, cz.michallebeda.diorama.engine.level.Floor floor, Player player) {
...
//        gameObjects.drawShadows(spriteBatch);
    ...
    public cz.michallebeda.diorama.engine.level.object.GameObject findIntersectingWithRay(ColorUtil colorUtil, Ray ray, Camera camera, Vector3 intersectionResult) {
        Vector3 intersection = new Vector3();
        BoundingBox boundingBox = new BoundingBox();

        float minDist = Float.MAX_VALUE;
        cz.michallebeda.diorama.engine.level.object.GameObject candidate = null;
        ...
        // Test ray against every game object we have
        Collection<cz.michallebeda.diorama.engine.level.portal.Portal> portalsCollection = portals.getPortals();
